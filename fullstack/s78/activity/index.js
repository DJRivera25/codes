...
console.log(isUserValid(users, "petra", "pass123"));
...
const borrowers = [
  ...
  { name: "Dazai", books: ["Gone Girl", `A Brief History of Time`] },
];

function getAvailableBooks(books, borrowers) {
  const borrowed = borrowers.flatMap((b) => b.books);
  console.log(borrowed);
  return books.filter((book) => !borrowed.includes(book.title));
}
console.log(getAvailableBooks(books, borrowers));
...
function getTotalSalesByDate(sales, date) {
  let salesOnDate = sales.filter((s) => s.date == date);
  console.log(salesOnDate);
  let totalSalesOnDate = 0;
  salesOnDate.forEach((s) => {
    totalSalesOnDate += s.quantity;
  });
  return totalSalesOnDate;
}
console.log(getTotalSalesByDate(sales, "2023-01-02"));
